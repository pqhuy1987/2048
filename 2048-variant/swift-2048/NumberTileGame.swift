...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
    ...
    func setupGame() {
        ...
        let bestScore = defaults.integer(forKey: defaultsKeys.best)
        bestScoreView.bestscore = bestScore > 0 ? bestScore : 0
    ...
    func scoreChanged(_ score: Int) {
        ...
        if bestScoreView == nil {
            return
        }
        
        let bv = bestScoreView!
        ...
        if score > bestScore {
            bv.bestScoreChanged(newScore: score)
            defaults.set(score, forKey: defaultsKeys.best)
